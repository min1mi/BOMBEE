...
-- 회원식단관리
DROP TABLE IF EXISTS MEM_MANAGE RESTRICT;
...
CREATE TABLE MEMB (
	...
	MTYPE       INTEGER      NOT NULL COMMENT '페북카톡일반' -- 페북카톡일반
)
COMMENT '회원';
...
CREATE TABLE PROMOTION (
	...
	TYPE    INTEGER      NOT NULL COMMENT '운동타입', -- 운동타입
	METER   INTEGER      NULL     COMMENT '거리' -- 거리
)
COMMENT '프로모션';
...
-- 회원식단관리
CREATE TABLE MEM_MANAGE (
	MENUNO    INTEGER      NOT NULL COMMENT '회원식단일련번호', -- 회원식단일련번호
	MEALTYPE  VARCHAR(255) NULL     COMMENT '끼니', -- 끼니
	MEALPIC   VARCHAR(255) NULL     COMMENT '식단사진', -- 식단사진
	...
	MEALKCAL  INTEGER      NULL     COMMENT '식단칼로리', -- 식단칼로리
	MEALNAME  VARCHAR(255) NULL     COMMENT '식단명', -- 식단명
	DAY       VARCHAR(45)  NULL     COMMENT '요일' -- 요일
)
COMMENT '회원식단관리';

-- 회원식단관리
ALTER TABLE MEM_MANAGE
	ADD CONSTRAINT PK_MEM_MANAGE -- 회원식단관리 기본키
		PRIMARY KEY (
			MENUNO -- 회원식단일련번호
		);
...
-- 회원식단관리
ALTER TABLE MEM_MANAGE
	ADD CONSTRAINT FK_TCHER_TRAINER_TO_MEM_MANAGE -- 개인트레이너 -> 회원식단관리
		FOREIGN KEY (
